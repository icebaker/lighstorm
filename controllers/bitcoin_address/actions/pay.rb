    ...
    module BitcoinAddress
      module Pay
        ...
        def self.adapt(response)
          Lighstorm::Adapter::Transaction.send_coins(response[:response])
        end

        def self.fetch(components, adapted, &vcr)
          Controllers::Transaction::All.data(components, hash: adapted[:hash], limit: 1, &vcr)
        end

        def self.model(data)
          Controllers::Transaction::All.model(data)
        end
